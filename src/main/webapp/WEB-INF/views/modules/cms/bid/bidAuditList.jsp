	...
	<script type="text/javascript">
		...
		function batPublish() {
			if($("#listForm input[name='ids'][st='2']:checked").length ==0){
				top.$.jBox.tip("请选择已送审的标书");
				return;
			}
			$("#listForm input[name='ids'][st='3']:checked").attr("checked",false);
			loading('正在提交，请稍等...');
	    	$("#listForm").attr("action", "${ctx}/cms/bid/audit?status=3");
	    	$("#listForm").submit();
    	}
		function batRefuse() {
			if($("#listForm input[name='ids'][st='2']:checked").length ==0){
			...
			$("#listForm input[name='ids'][st='3']:checked").attr("checked",false);
			loading('正在提交，请稍等...');
	    	$("#listForm").attr("action", "${ctx}/cms/bid/audit?status=4");
	    	$("#listForm").submit();
    	}
		function batOffline() {
			if($("#listForm input[name='ids'][st='3']:checked").length ==0){
				top.$.jBox.tip("请选择已发布的标书");
				return;
			}
			$("#listForm input[name='ids'][st='2']:checked").attr("checked",false);
			loading('正在提交，请稍等...');
	    	$("#listForm").attr("action", "${ctx}/cms/bid/audit?status=5");
	    	$("#listForm").submit();
    	}
	</script>
</head>
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/cms/bid/auditList">标书列表</a></li>
	</ul>
	<form:form id="searchForm" modelAttribute="bid" action="${ctx}/cms/bid/auditList" method="post" class="breadcrumb form-search">
	...
	<form id="listForm" method="post">
		...
		<table id="contentTable" class="table table-striped table-bordered table-condensed">
			...
			<th>标题</th><th>单位</th><th>来源</th><th>审核状态</th><th>创建日期</th><th>操作</th></tr></thead>
			<tbody>
			<c:forEach items="${page.list}" var="bid">
				<tr>
					<shiro:hasPermission name="cms:bid:edit">
						<td style="text-align: center; ">
							<c:if test="${bid.status eq 2 or bid.status eq 3}">
								<input type="checkbox" value="${bid.id}" name="ids" st = "${bid.status}">
							</c:if>
						</td>
					</shiro:hasPermission>
					...
					<td>
						...
						<shiro:hasPermission name="cms:bid:edit">
							...
							<c:choose>
							    <c:when test="${bid.status eq 2}">
							    	<a href="${ctx}/cms/bid/audit?ids=${bid.id}&status=3" onclick="return confirmx('确定要发布吗？', this.href)">发布</a>
									<a href="${ctx}/cms/bid/audit?ids=${bid.id}&status=4" onclick="return confirmx('确定要拒绝吗？', this.href)">拒绝</a>
									<span>下线</span>
								</c:when>
								<c:when test="${bid.status eq 3}">
									<span style="padding-right:4px;">发布</span><span>拒绝</span>
									<a href="${ctx}/cms/bid/audit?ids=${bid.id}&status=5" onclick="return confirmx('确定要下线吗？', this.href)">下线</a>
								</c:when>
							    <c:otherwise>
