	...
	<script type="text/javascript">
		...
		function batPublish() {
			if($("#listForm input[name='ids'][st='2']:checked").length ==0){
				top.$.jBox.tip("请选择已送审的新闻");
				return;
			}
			$("#listForm input[name='ids'][st='3']:checked").attr("checked",false);
			loading('正在提交，请稍等...');
	    	$("#listForm").attr("action", "${ctx}/cms/content/audit?status=3&chanId=${chanId}");
	    	$("#listForm").submit();
    	}
		function batRefuse() {
			if($("#listForm input[name='ids'][st='2']:checked").length ==0){
			...
			$("#listForm input[name='ids'][st='3']:checked").attr("checked",false);
			loading('正在提交，请稍等...');
	    	$("#listForm").attr("action", "${ctx}/cms/content/audit?status=4&chanId=${chanId}");
	    	$("#listForm").submit();
    	}
		function batOffline() {
			if($("#listForm input[name='ids'][st='3']:checked").length ==0){
				top.$.jBox.tip("请选择已发布的新闻");
				return;
			}
			$("#listForm input[name='ids'][st='2']:checked").attr("checked",false);
			loading('正在提交，请稍等...');
	    	$("#listForm").attr("action", "${ctx}/cms/content/audit?status=5&chanId=${chanId}");
	    	$("#listForm").submit();
    	}
	</script>
</head>
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/cms/content/auditList?chanId=${chanId}">新闻列表</a></li>
	</ul>
	<form:form id="searchForm" modelAttribute="content" action="${ctx}/cms/content/auditList?chanId=${chanId}" method="post" class="breadcrumb form-search">
	...
	<form id="listForm" method="post">
		...
		<table id="contentTable" class="table table-striped table-bordered table-condensed">
			<thead><tr>
				...
				<th>新闻标题</th><th>审核状态</th><th>创建人</th><th>创建日期</th><th>操作</th></tr></thead>
			<tbody>
						...
						<shiro:hasPermission name="cms:content:edit">
							<c:choose>
							     <c:when test="${tpl.status eq 2}">
							    	<a href="${ctx}/cms/content/audit?ids=${tpl.id}&status=3&chanId=${chanId}" onclick="return confirmx('确定要发布吗？', this.href)">发布</a>
									<a href="${ctx}/cms/content/audit?ids=${tpl.id}&status=4&chanId=${chanId}" onclick="return confirmx('确定要拒绝吗？', this.href)">拒绝</a>
									<span>下线</span>
								</c:when>
								<c:when test="${tpl.status eq 3}">
									<span style="padding-right:4px;">发布</span><span>拒绝</span>
									<a href="${ctx}/cms/content/audit?ids=${tpl.id}&status=5&chanId=${chanId}" onclick="return confirmx('确定要下线吗？', this.href)">下线</a>
								</c:when>
							    <c:otherwise>
