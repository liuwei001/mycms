...
<head>
	...
	<script type="text/javascript">
		...
		function batDelete(){
			...
			loading('正在提交，请稍等...');
	    	$("#listForm").attr("action", "${ctx}/cms/content/delete?channelId=${channelId}&type=1");
	    	$("#listForm").submit();
		}
		function batAudit() {
			if($("#listForm input[name='ids']:checked").length ==0){
			...
			loading('正在提交，请稍等...');
	    	$("#listForm").attr("action", "${ctx}/cms/content/toAudit?channelId=${channelId}&type=1");
	    	$("#listForm").submit();
    	}
	</script>
</head>
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/cms/content/spiderList">新闻列表</a></li>
		
	</ul>
	<form:form id="searchForm" modelAttribute="content" action="${ctx}/cms/content/spiderList" method="post" class="breadcrumb form-search">
...
<!-- 		</div> -->
	...
	<form id="listForm" method="post">
		...
		<table id="contentTable" class="table table-striped table-bordered table-condensed">
			...
			<th>新闻标题</th>
			<th>审核状态</th>
			<th>新闻来源</th><th>创建日期</th><th>操作</th></tr></thead>
			<tbody>
						...
						<shiro:hasPermission name="cms:content:edit">
							<c:choose>
							    <c:when test="${tpl.status eq 1 or tpl.status eq 4 or tpl.status eq 5}">
										<a href="${ctx}/cms/content/toAudit?ids=${tpl.id}&type=1" onclick="return confirmx('确定要送审吗？', this.href)">送审</a>
										<a href="${ctx}/cms/content/spiderForm?id=${tpl.id}">修改</a>
										<a href="${ctx}/cms/content/delete?ids=${tpl.id}&type=1" onclick="return confirmx('要删除该新闻吗？', this.href)">删除</a>
								</c:when>
							    <c:otherwise>
