...
<head>
	...
	<script type="text/javascript">
		...
		function batDelete(){
			...
			loading('正在提交，请稍等...');
	    	$("#listForm").attr("action", "${ctx}/cms/content/delete?chanId=${chanId}&parentId=${parentId}&type=0");
	    	$("#listForm").submit();
		}
		function batAudit() {
			if($("#listForm input[name='ids']:checked").length ==0){
...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/cms/content/list?chanId=${chanId}&parentId=${parentId}">新闻列表</a></li>
		<c:if test="${parentId > 0}">
			 <shiro:hasPermission name="cms:content:edit"> 
				<li><a href="${ctx}/cms/content/form?chanId=${chanId}&parentId=${parentId}">新闻添加</a></li>
			 </shiro:hasPermission> 
		</c:if>
	</ul>
	<form:form id="searchForm" modelAttribute="content" action="${ctx}/cms/content/list?chanId=${chanId}&parentId=${parentId}" method="post" class="breadcrumb form-search">
		...
		<input id="pageSize" name="pageSize" type="hidden" value="${page.pageSize}"/>
			...
			<label>创建日期：&nbsp;</label>
			<input id="beginDate" name="beginDate" type="text" style="width:120px;" readonly="readonly" maxlength="20" class="input-mini Wdate"
				value="<fmt:formatDate value="${content.beginDate}" pattern="yyyy-MM-dd"/>" onclick="WdatePicker({dateFmt:'yyyy-MM-dd'});"/>
			<label>&nbsp;--&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</label>
			<input id="endDate" name="endDate" type="text" style="width:120px;" readonly="readonly" maxlength="20" class="input-mini Wdate"
		...
		<table id="contentTable" class="table table-striped table-bordered table-condensed">
			...
			<c:forEach items="${page.list}" var="tpl">
				<tr>
					...
					<td style="max-width:550px; text-overflow: ellipsis;white-space: nowrap; overflow: hidden;" title="${tpl.title}">${tpl.title}</td>
					<%-- <td>
						${fns:getDictLabel(tpl.status, "cms_audit_status", "")}
					</td> --%>
					<td width="120">${tpl.author}</td>
					<td width="180"><fmt:formatDate value="${tpl.createDate}" pattern="yyyy-MM-dd HH:mm:ss"/></td>
					<td width="150">
						...
						<shiro:hasPermission name="cms:content:edit">
							<c:choose>
							    <c:when test="${tpl.status eq 1 or tpl.status eq 4 or tpl.status eq 5}">
										...
										<a href="${ctx}/cms/content/form?id=${tpl.id}&chanId=${chanId}&parentId=${parentId}">修改</a>
										<a href="${ctx}/cms/content/delete?ids=${tpl.id}&chanId=${chanId}&parentId=${parentId}&type=0" onclick="return confirmx('要删除该新闻吗？', this.href)">删除</a>
								</c:when>
							    <c:otherwise>
