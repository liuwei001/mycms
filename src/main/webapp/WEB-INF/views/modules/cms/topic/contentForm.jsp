...
<head>
	...
	<script type="text/javascript">
		...
		function view(){
			$("#inputForm").attr("action", "${ctx}/cms/content/view?chanId=-1");
			$("#inputForm").attr("target","_blank");
			$("#inputForm").submit();
			
			$("#inputForm").attr("action", "${ctx}/cms/topic/saveContent?topicId=${topicId }");
...
<body>
	<ul class="nav nav-tabs">
		<li><a href="${ctx}/cms/topic/topicContentList?topicId=${topicId }&func=1">专题新闻列表</a></li>
		<li class="active"><a href="${ctx}/cms/topic/contentForm?contentId=${content.id}&topicId=${topicId }">专题新闻<shiro:hasPermission name="cms:topic:edit">${not empty content.id?'修改':'添加'}</shiro:hasPermission><shiro:lacksPermission name="cms:topic:edit">查看</shiro:lacksPermission></a></li>
	</ul><br/>
	<form:form id="inputForm" modelAttribute="content" action="${ctx}/cms/topic/saveContent?topicId=${topicId }" method="post" class="form-horizontal">
		<form:hidden path="id"/>
		<form:hidden path="type" value="2"/>
		<form:hidden path="channelId" value="-1"/>
		<sys:message content="${message}"/>
		<div class="control-group">
			<label class="control-label">新闻所属专题:</label>
			<div class="controls">
				<label style="padding-top:3px;">${topicName }</label>
			</div>
		</div>
		<div class="control-group">
			<label class="control-label">专题内部分类:</label>
			<div class="controls">
				<select name="topicInnerChnlId" class="input-xlarge">
					<option value="">请选择</option>
					<c:forEach items="${topicInnerChnls}" var="tpl">
						<option <c:if test="${tpl.id eq topicInnerChnl }">selected="selected"</c:if>value="${tpl.id }" >${tpl.name }</option>
					</c:forEach>
				</select>
			</div>
		</div>
		...
		<div class="control-group">
			...
			<div class="controls">
				<form:input path="originUrl" htmlEscape="false" maxlength="50" class="input-xlarge"/>
			</div>
		</div>
		<div class="control-group">
