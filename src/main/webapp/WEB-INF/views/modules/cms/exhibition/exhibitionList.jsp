...
<head>
	...
	<script type="text/javascript">
		...
		function batDelete(){
			...
			loading('正在提交，请稍等...');
	    	$("#listForm").attr("action", "${ctx}/cms/exhibition/delete?chanId=${chanId}&type=0");
	    	$("#listForm").submit();
		}
		function batAudit() {
			if($("#listForm input[name='ids']:checked").length ==0){
			...
			loading('正在提交，请稍等...');
	    	$("#listForm").attr("action", "${ctx}/cms/exhibition/toAudit?chanId=${chanId}&type=0");
	    	$("#listForm").submit();
    	}
	</script>
</head>
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/cms/exhibition/list?chanId=${chanId}">展会列表</a></li>
		<c:if test="${chanId != null && chanId > 10}">
			<shiro:hasPermission name="cms:exhibition:edit">
				<li><a href="${ctx}/cms/exhibition/form?chanId=${chanId}">展会添加</a></li>
			</shiro:hasPermission>
		</c:if>
	</ul>
	<form:form id="searchForm" modelAttribute="exhibition" action="${ctx}/cms/exhibition/list?chanId=${chanId}" method="post" class="breadcrumb form-search">
						...
						<shiro:hasPermission name="cms:exhibition:edit">
							<c:choose>
							    <c:when test="${tpl.status eq 1 or tpl.status eq 4 or tpl.status eq 5}">
										<a href="${ctx}/cms/exhibition/toAudit?ids=${tpl.id}&chanId=${chanId}&type=0" onclick="return confirmx('确定要送审吗？', this.href)">送审</a>
										<a href="${ctx}/cms/exhibition/form?id=${tpl.id}&chanId=${chanId}">修改</a>
										<a href="${ctx}/cms/exhibition/delete?ids=${tpl.id}&chanId=${chanId}&type=0" onclick="return confirmx('要删除该新闻吗？', this.href)">删除</a>
								</c:when>
							    <c:otherwise>
